...
- `bin/clp-session turns` — where a Claude Code session's time went, per turn. A turn runs from one human prompt to the next (records the harness injects, such as task notifications, command output and compaction summaries, are not prompts), and its time splits into human wait (`AskUserQuestion`, `ExitPlanMode`), tool, model, idle (no event for `--idle-minutes`, default 10) and other, with no second counted twice. It reads whole records from an archive made by `clp-s-compress-session`, with or without `--structurize-arrays`, through `clp-s-search-kql`, and lists the longest tool waits. The record model follows [TraceLab](https://github.com/uw-syfi/TraceLab)'s Claude Code extractor (Apache-2.0): the assistant records that share a message id form one round, and a tool call is paired with its result by `tool_use_id`. Claude Code sessions only. Unit tests: `python3 -m unittest discover -s plugins/clp/tests`.
- `bin/clp-session facts` — computes every number a `claude-code-trajectory` report can quote, in code, so the report writer only puts facts into words. It reads a bundle's catalog and, through `clp-session turns`, `clp-bundle outcomes` and `clp-bundle repo`, writes a markdown facts file covering the skill's seven fixed categories (reliability, cost, time, outcomes, harness faults, human loop, rework), each figure with its denominator. `--axes` adds the scoring inputs: 20 axes in four groups, each with its raw value and the components behind it. It assigns no scores — the agent does that against `scoring-scale.json`, because what counts as an acceptable stall rate or cache hit rate is a customer's policy, not a measurement. `--axes --check-scale [--scale FILE]` validates a scale instead, printing `SCALE_OK` or one `SCALE_PROBLEM=` per defect. Its stdout is grep-able `KEY=VALUE` lines, one `AXIS <id> group=… value=… components=…` per axis (`value=` is a bare number or `n/a` with a reason, never a percent or a separator), and one `ALERT=<category>:<slug> value=… threshold=…` per category whose headline metric crosses a bad threshold, which is what the skill uses to order the focus options — alerts are not scores, and their thresholds are printed so they are never a hidden judgement. A category it cannot compute (no bundle, no repository) is reported as such, never as zero. Claude Code sessions only.
- `bin/clp-session score` — applies a scale to `clp-session facts --axes` output and writes the scores as JSON for a dashboard, plus `--format table` for a terminal. It maps each raw value to 0–10 through the scale's ladder and computes the per-group means, and does nothing a ladder lookup does not cover: no penalties, no weighting, no narrative, and no overall composite field anywhere in its output. That division is deliberate — measuring is `clp-session facts`, the thresholds are the customer's in the scale file, and what the scores *mean* stays with the agent. It resolves the scale by the same lookup order as the skill, records which one won in `scale.source`, and validates it first: a scale with defects gets its `SCALE_PROBLEM` lines printed, no JSON written, and exit 2, so a broken customer scale is never silently swapped for the default. An axis whose value is `n/a` is not scored — it goes to `unscored` with its reason and its group's mean rests on fewer axes, and a group with no scored axes has a `null` mean rather than 0. `--cohort KEY=VALUE` (repeatable) records what kind of work the session was, since a trend needs sessions grouped by similar shape; the JSON's `cohort._unset` lists the conventional keys left unsupplied so a consumer can tell an ungrouped session from one grouped as null. It also carries provenance — scale file and `scale_version`, bundle, and both tools' versions — so an old score stays interpretable after the scale moves.
- `scoring-scale.json` — the default 0–10 scale the `claude-code-trajectory` skill applies to `clp-session facts --axes` output: four groups with their owners, and per axis a label, what it measures, its unit, its direction and an exhaustive ladder. It is the only place default thresholds live. Every axis also states what its thresholds rest on: a `basis` of `definitional` (it follows from what the metric means), `mechanism` (from how the system works), `observed` (a named measurement, with its size — a single session says `n=1`) or `judgement` (a defensible choice with no measurement behind it), a `rationale` for where the rungs fall, and `anchors` for the values that pin them. Most defaults are `judgement`, and saying so is deliberate: it tells a customer which rungs to recalibrate first rather than implying an evidence base that does not exist. An axis may also carry a `gated_by` block naming a cohort key it depends on, so that an axis which is only meaningful under one intent is left unscored rather than scored misleadingly: `C4 autonomy` is gated on `supervision` (`autonomous`, `supervised` or `mixed`, declared in the scale's `cohort_keys`) and is scored only for a session meant to run unattended, since in a supervised session the same number is a design property and not a fault; `D5 cost concentration` is gated on `workload_shape` (`interactive`, `batch`, `mixed`) and scored only under `interactive`, because a batch or mixed workload can have one run dominate the bill by design. A gated-out axis lands in the scores JSON's `unscored` with `gated: true` and its reason, and its group's mean rests on one fewer axis; the raw value is still measured and reported, because the gate is a scoring policy and `clp-session facts` never applies it. An axis may also carry a `min_denominator`, below which it is left unscored with `insufficient_data: true` and both counts, because a rate over a handful of observations is noise whatever ladder reads it — `A5 runtime honesty` keeps a strict ladder and instead declines to rate fewer than 10 workflow instances, where one misreport would otherwise read as 50% dishonesty. `clp-session facts --axes` publishes each axis's `denominator=` where its divisor is genuinely a count of things (not where it is a duration), and the scores JSON carries it on scored axes too, so a consumer can weigh a score sitting just above its floor. The three reasons an axis goes unscored are therefore distinguishable: `gated` with its cohort key, `insufficient_data` with its counts, or neither when the value could not be measured at all. A top-level `calibration` object names how to replace them — `clp-bundle-review --json` over your own sessions gives the percentiles, within a cohort, with the caveat that one or two sessions describe those sessions and not a fleet. `--check-scale` rejects a scale whose axes lack a basis or rationale. A customer overrides it by naming a path, or with `./.clp-scoring-scale.json` or `.claude/clp-scoring-scale.json`, which take precedence in that order; the skill reports which scale it used and its `scale_version`.
- `bin/clp-s-schema-tree` — lists every field of an archive from its merged schema tree (`stats.schema_tree`), with no sampling: `FIELD path=… type=… records=N` per field, the KQL path first (an array element's fields are queried through the array, so `message.content[].name` is `message.content.name`; `shown=` gives the `[]` form). An object with many children that all look alike, such as data used as keys, is collapsed into one `*` row. With `--log-shapes-file` (a `stats.log_shapes` dump carrying `node_counts`), each text field also gets `values=` and `templates=`, and `TEXT_FIELDS=` lists the text fields, most values first. Two other readings of the same tree replace the field rows: `--drift` reports every field path the archive stores under more than one type (`DRIFT path=… types=ClpString:116,Object:4525 records=4641`, most records first) — the tree keeps one node per path and type, so a field that is a `ClpString` on a failed tool call and an `Object` on a success is already two nodes there, and a query written for one of the two shapes silently misses the records that carry the other; `--field-counts` reports the record root's children grouped by record count (`FIELDS count=32614 share=72.8% fields=… subtree_nodes=…`), where fields reading a co-equal count are a hint that the same records carry them — these are per-field counts and **not** a partition, since a record is counted under every field it carries, so the shares overlap and must never be summed, which the output says on itself in `FIELD_COUNT_NOTE=`. `--record-families` derives the partition those counts cannot give, and is the one mode that queries the archive. By default it partitions **by value** (`RECORD_FAMILY_METHOD=value field=type values=14 coverage=100.00% uncovered=0`): the values of a scalar field carrying at least `--min-partition-coverage` (0.99) of the records with at most `--max-partition-values` (64) distinct values, chosen by the same filter behind the bootstrap's `DIST` line — best coverage first, then shallowest, then fewest values — with one `--count FIELD:"<value>"` per value, quoted and escaped by the plugin's own rule (`lib/kql_build`) and numbers written bare so they match as numbers. That is exact and cheap: on a 44,818-record session archive it is `type`, 14 families, 15 queries, 2.5 s, residual 0. A field that misses a few records is used anyway, with the coverage always on the line so a partition of 99.97% never reads as one of everything: on the five-archive bundle 36 records of 111,223 lack `type`, and partitioning by it gives 18 families with those 36 in the residual for 19 queries and 5.4 s, where refusing it over them costs 716 queries, 44 s and a residual of 3,726. Where no field reaches the coverage threshold it falls back to partitioning **by existence** (`RECORD_FAMILY_METHOD=existence reason=…`, naming the closest near-miss, and `--family-method existence` forces it and then also prints `RECORD_FAMILY_COMPARE` so the approximation is visible): one discriminator per child subtree of the record root (a leaf every record of that subtree carries; a scalar child is its own; a subtree no single leaf covers gets a disjunction of its leaves, `kind=structural-disjunction`, whose predicate is in the NDJSON), every pair proved disjoint with a `--count` query. Either way it reports `RECORD_FAMILY … count=16604 share=37.1%` per family plus a `RECORD_FAMILY_RESIDUAL` for the records no family matched, so the shares and the residual sum to 100% — an invariant checked on the counts, which fails the run before anything is printed. A candidate that is not disjoint from a family is reported with the relation that ruled it out (`superset of message.role` for a `uuid` every kind of record carries), a subtree that not even a disjunction covers is reported as undiscriminated with the count it reached, and `--max-family-queries` (default 1000 — about 520 queries and half a minute on a 44,818-record session archive) is a ceiling for pathological archives rather than a budget to tune down: when it does bind, `cap=hit`, `untested=` and `RECORD_FAMILY_CAP_NOTE=` say so and that the residual is then only an upper bound. `also=` on a family names fields a query proved select the very same records, never fields that merely read the same count — a session archive has three kinds of record numbering exactly 2,249 and they are nothing alike. `--drift-file`, `--field-counts-file` and `--record-families-file` write the same rows as NDJSON.
...
- `bin/clp-insights bootstrap` — one-command bootstrap for the `log-insights` skill: the schema tree's fields (`FIELD`, `TEXT_FIELDS=`), value distributions for the most common scalar fields from a sample, the fields each template came from (`TEMPLATE_FIELDS_FILE=`), log shape dictionary dump, per-template frequencies from the counts stored in the archive, and the classification-cache probe, summarized as grep-able `KEY=VALUE` lines. Before the dump it reads `stats.archives` — a tenth of a second against the archive headers — for the templates and variables the archives actually store (`ARCHIVE_LOG_SHAPES=`, `ARCHIVE_VARS=`, one `ARCHIVE_STAT` per archive), states that count in its first progress line and sizes its estimate with it; and from the schema tree it already dumped it adds the type drift and the per-field record counts (`TYPE_DRIFT_COUNT=`, `TYPE_DRIFT_FILE=`, `FIELD_COUNT_GROUPS=`, `FIELD_COUNT_NOTE=`, `FIELD_COUNTS_FILE=`), where `TYPE_DRIFT_COUNT=0` is the answer "nothing drifts" and `UNAVAILABLE` means there was no tree to read. It then spends a few hundred counting queries — tens of seconds, narrated as it goes — on the verified partition of the records (`RECORD_FAMILY_COUNT=`, `RECORD_FAMILY_RESIDUAL=`, `RECORD_FAMILY_QUERIES=`, `RECORD_FAMILIES_FILE=`), which is the one part of the summary whose shares may be summed; it degrades to `RECORD_FAMILY_COUNT=UNAVAILABLE` with a hint if those queries cannot run.
...
- `bin/kql-build` (+ `lib/kql_build.py`) — renders a structured JSON filter (`all`/`any`/`not`, `eq`/`contains`/`prefix`/`exists`/`gt`…, scoped `semantic`) to KQL with every value quoted and escaped and every group parenthesized, so query plans never carry model-written KQL. `kql-build render '<filter>'` prints the KQL; `kql-build check-plan FILE` validates every query_plan entry, and its ranking (`lib/classification.py`): each entry's `category`, `priority` and `stage`, and each taxonomy category's `priority` and `why`. `log-shape-cache merge`/`put` refuse classifications that fail the same check. With `--drift-file F` (a `clp-s-schema-tree --drift-file` dump) it also catches a filter that will silently answer about part of its field: clp-s stores a node per type, and a scalar filter reaches the union of a path's scalar types but never its `Object` or `StructuredArray` nodes — `toolUseResult:*` matches 116 records, not the 4,641 carrying that path, because the other 4,525 hold an object. Each at-risk entry gets a `DRIFT_RISK` line with the type counts, the direction measured (`at=path` or `at=descendant`, since the reachable share differs by where the filter sits) and the `addressable` share; the entry fails when that share is below `--drift-fail-below`, default 0.95. A filter may declare `"types": [...]` on a leaf to say it targets one shape deliberately, which reports `DRIFT_DECLARED` and never fails, while a declaration naming a type the path does not hold — or naming only types a filter there cannot reach — is rejected as a plan error; there is no flag to silence the check. Without `--drift-file` it prints `DRIFT_CHECK=SKIPPED`, so a missing dump never reads as a clean pass. The direction split also catches a dead query: a filter on a subfield of a path that is never structural reaches nothing at all, and `toolUseResult.stdout.lines:*` matching zero records previously failed silently. `kql-build validate-wildcards` is the search wrapper's guard against an unquoted wildcard value with a space (`field:*a b*`, which clp-s reads as natural language), and rejects the query before it runs.
...
A bundle is one session's logs kept as CLP archives, with a manifest and a SQLite catalog: `archives/` (one clp-s archives dir, one archive per kind of log: main log, direct-agent transcripts, workflow-agent transcripts, workflow journals, workflow summaries), `files/` (every other file of the session, such as agent meta files, tool results, file snapshots and tasks, at its path relative to the session), `manifest.json` (every source file: kind, size, hash, and where it went, down to the positions of its records in its archive) and `catalog.sqlite` (what exists and how it connects: agents, workflow runs and their resumed instances, retried attempts with a failure cause, timing, and one row per record). The catalog is built from the bundle alone: each archive is read once, in its original order, using the position clp-s keeps for each record, so `clp-bundle BUNDLE rebuild` makes it again without the session's original files. It names records by the IDs they carry (`agentId`, `runId`, `uuid`) and holds no text. It also holds token usage as the API reported it, each model response counted once: totals for the main thread, every agent and attempt, and every workflow instance and run (`tokens_*` on nodes), and per call on the record that carries each response's final usage (`tokens_*` on events; `tokens_input` is the context sent with that call). A workflow run's own figure, which is closer to its final context size, is kept apart as `attrs.runtime_tokens`. `clp-session turns` reports the main thread's tokens in total and per turn, and with `--calls` one line per call.

`clp-bundle build` makes one from a Claude Code session. Every file of the session is classified by its path, and one that matches no rule stops the build; so does a line that is not a JSON record, or a last record cut off mid-write (for a session still running, build it once it has stopped), each named by file and line. NUL bytes, which lost writes leave in real session logs and which clp-s cannot read, are removed before compression (a raw NUL byte is never part of valid JSON); each file's count is printed as `REPAIRED` and kept in the catalog, and the source files are never changed. The build checks that each archive holds exactly the records its files contribute, and removes what it made if anything fails. It refuses an existing directory (`--force` replaces an existing bundle only). A session with no subagents or workflows gets a catalog with only its main thread. A `uuid` can occur twice, since Claude Code rewrites some records with the same `uuid` when a session is reopened; `who --uuid` and `record` show every copy. A catalog of an older layout is refused with the command that rebuilds it. `clp-bundle` moves between the catalog and the archives:
...
./plugins/clp/bin/clp-bundle BUNDLE rebuild                            # make catalog.sqlite again from the bundle alone
...
./plugins/clp/bin/clp-bundle BUNDLE repo [--repo PATH] [--no-github]    # which commits (git, all refs and reflogs) and PRs (GitHub) each command actually made, matched by time
...
`clp-bundle-review DIR --build [--skip ID] [--json]` reviews many sessions at once, for someone looking at the harness rather than one task: it bundles every Claude Code session under a Claude home into DIR (building missing or changed ones, rebuilding catalogs of an older layout, skipping a session still being written), computes a signal vector per session, and reports zero-tolerance findings (damaged logs, tool calls without a result, workflow runs reported completed with failed attempts, resumes re-running finished work, configuration errors, ...) with example ids, rate signals against the median and 90th percentile of the sessions active enough to rate, the sessions flagged most often, and failed tool calls grouped across sessions. Groups start from CLP's own templates (`shape(toolUseResult)`); command failures are then merged by a shared beginning and end, and other messages by embeddings from the plugin's built-in semantic endpoint (their template text is sent to it; `--semantic-endpoint` names another), and an unreachable endpoint is an error. `--trend week|day` adds the signals per ISO week or day, summed over all sessions by each event's own time, with volume, the number of sessions behind each value and a 95% interval for rates; a change is flagged only between periods with enough volume whose intervals do not overlap.
...
```bash
./plugins/clp/bin/clp-insights bootstrap /tmp/archive
```

It samples records for schema discovery, prints per-field value distributions, dumps + normalizes the dictionary, sums the per-template counts that clp-s stored at compression time, probes the classification cache, and prints a grep-able `KEY=VALUE` summary (`LOG_SHAPE_COUNT=`, `FREQS=`, `CACHE_MODE=`, `CACHE_REASON=`, `TO_CLASSIFY=`, `MAX_CHARS=`, output-file paths). The cache probe is given the field rules, which are part of its key: `--field-rules F`, or the rules file an earlier run of the same `--out-dir` left behind (`RULES_SOURCE=`, `RULES_FILE=`, `RULES_DIGEST=`). The rules are derived after the bootstrap, so a first run has none to probe with and `CACHE_REASON=rules-changed` is how a later run says the cached classification was built under other rules — a re-classification, not a first run. The first time it sees an archive it renders the dictionary once (`log-shape-cache ingest`) and stores each template's hash, count, length and first `MAX_CHARS` characters in the cache database; a later run on the same archive (`SHAPES_SOURCE=stored`) reads them back and skips the dump. It first prints an estimate from the archive's size (`ESTIMATE_SECONDS=`: about 1 minute per 300 MiB of archive on a first run, about the sample's time after), then a `[bootstrap]` line as each of its three stages starts and ends, a heartbeat every 30 s while one runs (`--heartbeat`), and the measured `BOOTSTRAP_TIMINGS` at the end. `FREQS=UNAVAILABLE` means an archive was compressed before clp-s stored those counts; recompress it to get frequencies.
...
The applied **field rules** are in the key too, as a canonical digest folded into it (`--field-rules` on `key` and `diff`; `rules-digest` prints the digest alone, and `log-shape-cluster` prints the same value as `RULES_DIGEST=`, from the same shared helper). A ruled template takes its category from the rule and is never embedded or shown to the classifier, so the rules are part of the classification: changing a rule, adding or dropping one, or re-running `fields --propose-rules` with another `--free-text-ratio` re-keys the cache instead of silently reusing categories no rule would give now. The digest is canonical — sorted by field path, only each rule's `field` and `category`, so re-ordering the file or dropping the ratios and counts `--propose-rules` annotates it with changes nothing — and having no rules at all is itself a rule set, with a stable digest of its own.

The cache is one SQLite database, `cache.sqlite` in the cache directory. Each entry holds the schema, taxonomy and query plan, and one row per template: its `hash` (sha256 of the full template), its `prefix_hash` (sha256 of the first `--max-chars` characters) and its category. Templates are stored by hash, never by text: some apps log templates of hundreds of KB (CockroachDB's Pebble stats tables average ~184 KB), and an entry that held their text grew to 2.28 GB, which every lookup parsed in full. That entry is 2.2 MB as rows, and looking it up, storing it or merging into it each takes well under a second. The text stays in the archive's own dictionary dump; `clp-insights extract` joins it on the hash. The shared hashing lives in `bin/lib/log_shapes.py`.
...
"$LC" diff  --log-shapes-file /tmp/log-shapes.ndjson \
      --field-rules /tmp/log-shape-field-rules.json    # UPTODATE | GROWTH | NEW + a reason
"$LC" rules-digest --field-rules /tmp/log-shape-field-rules.json  # the digest the key folds in
...
| Header | Meaning |
| --- | --- |
| `UPTODATE\t<app_key>\t<count>\tup-to-date` | Fingerprint unchanged — reuse the cached classification, nothing to classify. A template that differs from a cached one only past the character limit takes its category through the shared prefix hash. |
| `GROWTH\t<app_key>\t<base_key>\t<count>\t<new_count>\ttemplates-grown` | Archive grew from `<base_key>` — only the `<new_count>` new templates follow and need classifying. |
| `NEW\t<app_key>\t<count>\tfirst-run` | No compatible base — all `<count>` templates follow. |
| `NEW\t<app_key>\t<count>\trules-changed` | The same, but an entry does hold these templates under other field rules: the rule set changed, so the app is classified again. Not a first run — the bootstrap passes this through as `CACHE_REASON=` so the skill can say which. |

The last field of every header is that reason. `<count>` is always the TRUE full template count, not the de-duplicated one. The GROWTH subset test is one indexed query over the prefix hashes, so a change that only affects bytes past the character limit reports UPTODATE rather than GROWTH.
...
The database carries a format version, and entries written in an older one are **discarded, not migrated**: an entry keyed before the field rules were part of the key cannot be told apart from one classified with no rules, and nothing records which rules produced it. The first command to open such a cache says how many classifications it dropped, in which database, and that those apps are classified again on their next run, and leaves a snapshot of the old database beside it (`cache.sqlite.format1.<timestamp>`). The stored archives survive, so nothing is dumped again for it.

Cache location: `~/.config/yscope-clp-plugin/log-shape-cache/`, overridable with `$CLP_LOG_SHAPE_CACHE_DIR`, or per-command with `--cache-dir` on the subcommands that read or write the cache (`freqs`, `ingest`, `stored`, `diff`, `get`, `merge`, `put`, `forget`, `list`, `show`). `normalize`, `count`, `key` and `rules-digest` only transform/hash the input and do not accept it. `--max-chars` (default 500, or `$CLP_LOG_SHAPE_MAX_CHARS`) is accepted by the subcommands that compute or stamp the fingerprint or cut the stored prefixes (`key`, `ingest`, `stored`, `diff`, `put`); it must match the limit given to `log-shape-cluster`, or embedding and cache fingerprints diverge. `--field-rules` is accepted by `key`, `diff` and `rules-digest`, and must be the rules file `log-shape-cluster` is given, for the same reason.
...
`fields --propose-rules FILE` derives those rules instead of reading them off a summary, from the one mechanical test that decides it: a field whose template count approaches its value count is free text, where every value became its own template, so clustering groups nothing and embedding buys nothing. It proposes a rule for each field whose ratio is at least `--free-text-ratio` (0.8), or that has at least `--free-text-templates` (500) templates at a ratio of at least `--free-text-floor` (0.5) — the second test catching diff lines, which repeat enough to sit at 0.68 while plainly being free text — and never for a field with fewer than `--free-text-min-templates` (5) templates, since a rule saves nothing there and JSON that uses data as keys makes a one-template field per datum. Each rule carries `proposed_by`, the ratio and the counts, so the analyst sees what is about to be ruled and can drop or rename it; the category is derived from the field and has to be added to the taxonomy like any other. Nothing is applied — the file is written and a `{"summary": …}` line measures what it would rule. On that same session it proposed 39 rules covering 24,604 of 25,483 templates, leaving 879 to embed in 388 clusters, against the 8,278 the hand-picked rules left.
