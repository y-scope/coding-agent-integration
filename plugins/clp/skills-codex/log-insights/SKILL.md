...
10. Close with a short message: three to five findings, most important first, with inferences labelled; the caveats that change how to read them; and where the report is. Do not restate the report. Then offer at most three next steps, one line each: drill deeper on a finding or another category's drill entries, note that re-running on the same application skips classification (cached plan reused), or decompress: `~/.codex/marketplaces/yscope/plugins/clp/bin/clp-s-decompress <archives-dir> <out-dir>`.
