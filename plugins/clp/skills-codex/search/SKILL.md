...
- Project only the columns you need to limit data returned: when you know which fields matter, pass `--projection COLUMNS` (comma-separated; repeatable), e.g. `--projection timestamp,payload.name`. `clp-s` returns leaf columns only, and an array is a leaf that comes back whole: a column inside an array, or an object, projects nothing. The wrapper rewrites such a column to the array that holds it, or to the leaf columns under the object, and says so on stderr (`projection: A -> B`), so expect the array itself in the output, e.g. `payload.content` for `payload.content.text`. Omit it only when you need the full record.
