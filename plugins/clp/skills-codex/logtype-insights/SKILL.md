...
3. **Bootstrap.** Tell the user you are sampling the schema, dumping the logtype dictionary, and probing the classification cache — then run the one command that does all of it (it also handles the templatize fallback for binaries that predate the shapes API):
...
From its `KEY=VALUE` output record:
   ...
   - `FALLBACK=TEMPLATIZE_NEEDS_MESSAGE` → the clp-s binary predates the shapes API; tell the user ("old clp-s binary — rebuilding the baseline via message templatization"), then re-run ONCE adding `--message <message-field>` (from SAMPLE/DIST).
...
7. **Run the insight pass** (inline, from `/tmp/logtype-classification.json`). Announce it first ("running the insight pass — executing the K planned queries; may take a few minutes"). As you go, give one-line updates: the count after each query-plan entry (or small group of entries), one line after the frequency pass, and "queries done, writing the report" before step 8. Execute every `query_plan` entry:
   - `count`: run the KQL and `grep -c '^{'`.
   - `project+grep`: run the KQL with `--projection`, then `grep '^{' | jq -r '.<message>' | grep -Ei '<grep>'`.
...
Then:
   - **Per-template frequencies** (the count baseline): project the message field, templatize, `uniq -c`:
     ```bash
     ...
     "$S" --projection <message> <archive-dir> '*' \
       ...
       | sort | uniq -c | sort -rn
     ```
   - **Total records**: `*`. **Severity/logger breakdowns**: reuse the bootstrap DIST lines (they cover the sampled records; re-count per value with KQL when exact totals matter). **Time span**: project the timestamp field and use `head`/`tail` (chronological; do NOT sort), or `--tge`/`--tle` if the timestamp is a real epoch.
   - `<message>:term` is an exact match, so it correctly returns 0 unless a message equals exactly `term`. Exact match is faster, so use it when you know a field's full value; message content is free text and almost always needs a substring wildcard — `<message>:*term*`. Combine with a scalar filter in one compound query when you can (`<severity>:<value> AND <message>:*term*`, `<logger>:*<substr>* AND <message>:*term*`). Fall back to projecting message + grep only when the match needs a regex.
