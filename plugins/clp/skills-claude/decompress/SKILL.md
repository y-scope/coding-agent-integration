...
- Accept the top-level `Archives dir` printed by compression, or the inner `clp-s` archive directory. Prefer the top-level path.
...
clpp (clp+ / `--experimental`) archives are decompressed the same way: the wrapper auto-detects them (by their `parsing_specification` / `parent_rule_shapes` marker files) and passes `--experimental` for you — no extra flags needed. clpp archives require `--experimental` to open, so this auto-detection is required; pass `--no-experimental` only to force stable mode. See `clpp-shared.md` for clpp background.
