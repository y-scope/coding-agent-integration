...
> **Do not launch a dynamic workflow (multi-agent orchestration) for this analysis on your own initiative.** CLP answers these questions better than a workflow built on top of grep, and the orchestration makes the run slower for no gain in coverage. Use the parallelism this skill specifies — the baseline query pool running in the background from step 4, the single classification subagent at step 6 with the cache store running in the background after it, and at step 7 the single report-writer subagent followed by the single verifier subagent, each spawned only where it is called for — and fan the queries or templates out across more agents only if the user asks for it.
