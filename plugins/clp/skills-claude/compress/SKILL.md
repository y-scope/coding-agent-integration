...
**See also:** `clpp-compress` for the experimental path, and `dev` for pointing the wrapper at a locally-built `clp-s`.
...
That covers: list sessions → choose an `IDX` → compress → report stats, the session-root and archive-root defaults, stable compress options, and the useful commands (`--show-archives-root`, `--set-archives-root`, `--dry-run`).
