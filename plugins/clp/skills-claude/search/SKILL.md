...
Search a local CLP archive with KQL using the **stable** (non-experimental) clp-s path.

Use only the plugin wrapper. Do not call bare `clp-s`.

**When to use this skill:** plain (non-experimental) KQL / semantic search. If the user mentions clp+, clpp, or clp-s experimental — or wants `shape()` / `decompose()` / log shapes / decomposed-query projections — use the `clpp-search` skill instead.
...
That covers: the KQL table, the wildcard-substring rule (literals match whole values; substrings need explicit `*`), the time-range-flag gotcha, array dot-notation, semantic search, `--projection`, and counting with `grep -c '^{`.
