"""logtypes - the character limit and hashing shared by the logtype-* helpers.

A logtype (message template) can be hundreds of kilobytes long: CockroachDB logs
...
and the classification files identify each template by two hashes:
  ...
  prefix_hash  sha256 of its first max_chars characters, which is what
               logtype-cluster embeds and what the cache fingerprint is built
...
logtypes and freqs files); whoever needs it joins on `hash`.
...
# context (see logtype-cluster.py). Changing it deliberately re-keys the cache.
DEFAULT_MAX_CHARS = 500


def default_max_chars():
    try:
        value = int(os.environ.get("CLP_LOGTYPE_MAX_CHARS", DEFAULT_MAX_CHARS))
